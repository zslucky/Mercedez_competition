...
# param_grid = [
#   {'n_estimators': [50, 100, 125, 150, 250]},
#   {'learning_rate': [1, 0.5, 0.3, 0.05, 0.03]}
# ]
...
for foldID, (train_index, valid_index) in enumerate(kf.split(train)):
  ...
  reg = AdaBoostRegressor(**ab_params)
...
feature_dataframe = pd.DataFrame({
  'features': cols,
   'AB_feature_importances': average_importances
  })

importances_df = feature_dataframe.sort_values('AB_feature_importances', ascending=False)
...
sub.to_csv('adaboost_test_model.csv', index=False)

'''
'''
mode_random_forest = pd.DataFrame({'ID': id_train,'y': y_valid_preds})
mode_random_forest.to_csv('adaboost_feature_model.csv', index=False)