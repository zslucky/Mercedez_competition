...
# param_grid = [
#   {'loss': ['ls', 'huber']},
#   {'n_estimators': [100, 125, 150, 200, 250, 350]},
#   {'learning_rate': [0.1, 0.3, 0.5, 0.03, 0.05]},
#   {'max_depth': [3, 4, 5, 6]},
...
for foldID, (train_index, valid_index) in enumerate(kf.split(train)):
  ...
  reg = GradientBoostingRegressor(**gb_params)
...
feature_dataframe = pd.DataFrame({
  'features': cols,
   'GB_feature_importances': average_importances
  })

importances_df = feature_dataframe.sort_values('GB_feature_importances', ascending=False)
...
sub.to_csv('gradient_boost_test_model.csv', index=False)

'''
'''
mode_random_forest = pd.DataFrame({'ID': id_train,'y': y_valid_preds})
mode_random_forest.to_csv('gradient_boost_feature_model.csv', index=False)